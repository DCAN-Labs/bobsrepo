...
abcd_hcp_pipeline_template_anatonly="template_anatonly.abcd-hcp-pipeline_infant"
...
if [ ! -d "${run_folder}/output_logs" ]; then
	mkdir "${run_folder}/output_logs"
fi

# counter to create run numbers
k=0

file='sublists/sublist_anatonly_latest.csv'
while IFS=',' read -r subid sesid rest_of_line; do
	sed -e "s|SUBJECTID|${subid}|g" -e "s|SESID|${sesid}|g" -e "s|TEMPDIR|${temp_dir}|g" -e "s|BUCKET|${data_bucket}|g" -e "s|RUNDIR|${run_folder}|g" ${run_folder}/${abcd_hcp_pipeline_template_anatonly} > ${abcd_hcp_pipeline_folder}/run${k}
	k=$((k+1))
done < "$file"


file='sublists/sublist_latest.csv'
while IFS=',' read -r subid sesid rest_of_line; do
